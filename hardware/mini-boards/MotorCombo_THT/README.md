...
- **Pull-up:** JP1 P2–P3 closed, JP2 closed, R2 position populated with a **bridge** (a 0 Ω resistor in the same footprint, or a wire link) — the pull-up is active and SIG-OUT is passed through. **Important:** do not leave R2 open here. The board has no separate direct GND connection, so the R2 position provides the ground for the smoothing capacitor; left open, the ceramic cap would sit in series in the signal path instead of to ground.
